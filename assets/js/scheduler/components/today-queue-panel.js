...
function nowRemaining(session) {
  if (!session) return 0;
  let remaining = Number.parseInt(session.remaining_sec, 10) || 0;
  if (Number.parseInt(session.running, 10) !== 1 || !session.started_at) return Math.max(0, remaining);
  const startMs = Date.parse(session.started_at);
  if (Number.isNaN(startMs)) return Math.max(0, remaining);
...
export class TodayQueuePanel extends HTMLElement {
  connectedCallback() {
    ...
    this.render();
    this.tick = setInterval(() => this.renderNowOnly(), 500);
  ...
  renderNowOnly() {
    ...
    if (!session || !activeTask) {
      if (session?.mode === "break") {
        const rem = nowRemaining(session);
    ...
    this.barNode.style.transform = `scaleX(${ratio})`;
    if (session.running && !reduceMotion) this.barNode.classList.add("is-running");
    else this.barNode.classList.remove("is-running");
  }
