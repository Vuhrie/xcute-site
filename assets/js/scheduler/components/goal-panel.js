...
template.innerHTML = `
  <section class="x-panel">
    ...
    <div class="x-row x-edit-box">
      <label>Selected goal settings</label>
      <div class="x-inline">
        <input type="number" name="edit_daily_hours" min="1" max="16" />
        <input type="date" name="edit_target_date" />
        <button class="c-btn" data-action="save-selected">Save Goal</button>
      </div>
    </div>
...
export class GoalPanel extends HTMLElement {
  ...
  async onClick(event) {
    ...
    try {
      ...
      if (action === "save-selected") {
        const selected = getState().selectedGoalId;
        if (!selected) return;
        await updateGoal({
          id: selected,
          daily_hours: Number.parseInt(this.querySelector('[name="edit_daily_hours"]').value, 10) || 2,
          target_date: this.querySelector('[name="edit_target_date"]').value || null,
        });
  ...
  render() {
    ...
    this.querySelector('[name="edit_daily_hours"]').value = selected?.daily_hours || 2;
    this.querySelector('[name="edit_target_date"]').value = selected?.target_date || "";
  }
}
